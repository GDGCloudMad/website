...
class Bundler extends stream_1.Transform {
    constructor(root, entrypoint, shell, fragments, analyzer) {
    ...
    _buildBundles() {
        return this._getBundles().then((bundles) => {
            let sharedDepsBundle = (this.shell)
                ? path_transformers_1.urlFromPath(this.root, this.shell)
            ...
            for (let fragment of this.allFragments) {
                let fragmentUrl = path_transformers_1.urlFromPath(this.root, fragment);
    ...
    _addSharedImportsToShell(bundles) {
        ...
        let shellUrl = path_transformers_1.urlFromPath(this.root, this.shell);
        let shellUrlDir = path_1.posix.dirname(shellUrl);
        let shellDeps = bundles.get(shellUrl)
            .map((d) => path_1.posix.relative(shellUrlDir, d));
        ...
        let imports = dom5.queryAll(doc, dom5.predicates.AND(dom5.predicates.hasTagName('link'), dom5.predicates.hasAttrValue('rel', 'import')));
        ...
        return this.analyzer.analyzeDependencies.then((indexes) => {
            ...
            for (let fragment of fragmentToDeps.keys()) {
                ...
                for (let dep of dependencies) {
                    ...
                    if (fragmentCount > 1) {
                        if (this.shell) {
                            addImport(path_transformers_1.urlFromPath(this.root, this.shell), dep);
                        }
                        else {
